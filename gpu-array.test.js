...
TEST("Operator", [
    ...
    ["a + b -> c (assign)", async () => {
        const a = gpu.full(2, { shape: [2, 2] });
        const b = gpu.full(1.7, { shape: [1] });
        const c = gpu.Array({ shape: [2, 2] });
        const d = gpu.add(a, b, c);
        assertTruthy(Object.is(c, d));
        assertTruthy(c.gpu_dirty);
        await c.load();
        assertFalsy(c.gpu_dirty);
        assertAlmostEqual(c, [3.7, 3.7, 3.7, 3.7]);
    }],
    ["a - b", async () => {
        const a = gpu.full(2, { shape: [2, 2] });
    ...
    ["a (scalar) + b (scalar)", async () => {
        const c = gpu.add(1.5, 2.2);
        await c.load();
        assertEqual(c.shape, [1]);
        assertAlmostEqual(c, [3.7]);
    }],
    ["a + b (scalar) -> c (Broadcast / assign)", async () => {
        const a = gpu.full(1.1, { shape: 2 });
        const c = gpu.Array({ shape: [2, 2] });
        gpu.add(a, 0.7, c);
        await c.load();
